---
name: paper-review
description: 以资深审稿人视角审视论文，产出顶会风格审稿报告（Summary/Strengths/Weaknesses/Rating）+ 改稿策略建议（问题根源、可救性判断、行动指南）。支持上传 PDF 或直接审当前文本。触发场景：审稿、reviewer 视角、模拟审稿意见、这篇能中吗、给改稿建议、投稿前评估。
---

# Reviewer 视角审稿（paper-review）

模拟顶会审稿人对论文做客观、严苛但具建设性的评估。

## 输入方式

- **有 PDF**：用户上传论文 PDF，按整篇评估。
- **无 PDF**：直接审视当前对话中的论文文本/段落（被 `paper-workflow` 调用时即此模式）。
- 询问或确认用户的**投稿目标**（如 ICML 2026），评分与建议需对标该会议。
...
# Task
请深入阅读并分析我上传的【PDF论文文件】（若无 PDF，则分析当前提供的论文文本）。基于我指定的【投稿目标】，撰写一份严格但具有建设性的审稿报告。
...
# Input
请根据上传的 pdf 附件（或当前提供的论文文本）进行分析，我计划投稿于 [投稿目标，例如：ICML 2026]
```
...
`paper-workflow` 的阶段3调用本 skill（无 PDF 时审当前文本）。Part 2 Strategic Advice 是最终输出「修改方向」的主要来源，Part 1 的 Weaknesses 汇入「修改意见」。
