---
name: paper-figure
description: 论文配图辅助。两类能力：①根据方法描述生成可喂给图像模型（如 nano-banana）的论文架构图 prompt（中/英两版）；②根据实验数据从 19 类标准学术图表库推荐最合适的绘图方案与视觉设计规范。本 skill 产出 prompt 与推荐，不直接画图。触发场景：画架构图、生成框架图 prompt、这组数据用什么图、推荐图表类型、实验绘图。
---
...
- 用户给**方法描述 / 摘要**，想要架构图 → **模式 A：论文架构图 prompt 生成**
- 用户给**实验数据 / 实验目的**，问用什么图 → **模式 B：实验绘图类型推荐**

---

## 模式 A：论文架构图 prompt 生成

产出可直接喂给图像生成模型的架构图 prompt。提供中、英两版，建议都试、按审美取优（英文版对 nano-banana 类模型常更稳）。
...
# Input Methodology
[用户提供的论文摘要(Abs) + 方法部分描述]
```

### 英文版（对 nano-banana 等图像模型常更稳）
...
根据实验数据/目的，从下方标准学术图表库推荐 1–2 种最佳方案。配色可参考 https://htmlcolorcodes.com/zh/yanse-xuanze-qi/ 。审美判断具主观性，LLM 推荐仅供参考。
...
# Input
[用户提供的实验数据（推荐直接复制 Excel/CSV 原始表格，保持行列结构），并简述想通过这张图强调的核心结论]
```
